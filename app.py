...
if not selected_data.empty:
    for idx, row in selected_data.iterrows():
        try:
                    ...
                    if st.button(f"Run Vina Docking", key=f"dock_{idx}"):
                        with st.spinner("Preparing docking pipeline..."):
                            ...
                            if receptor_pdbqt and ligand_pdbqt:
                                ...
                                vina_output, error_msg = run_vina_docking(receptor_pdbqt, ligand_pdbqt, [cx, cy, cz], [sx, sy, sz])

                                if error_msg:
                                    st.error(f'Vina Execution Failed: {error_msg}')
                                else:
                    ...
                    <script>
                        var initViewer_{idx} = setInterval(function() {{
                            if (typeof $3Dmol !== 'undefined') {{
                                ...
                                var receptor_data = {json.dumps(receptor_data)};
                                var ligand_data = {json.dumps(selected_pose_str)};
            ...
            st.error(f'An error occurred while processing {row["Common Name"]}: {e}')
